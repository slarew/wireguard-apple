...
) -> CollectionDifferenceBackport<C.Element>
  where
    ...
    C.Element == D.Element
{
  ...
  func _formChanges(
    ...
    using trace: [_V]
  ) -> [CollectionDifferenceBackport<C.Element>.Change] {
    var changes = [CollectionDifferenceBackport<C.Element>.Change]()
  ...
  return _withContiguousStorage(for: old) { a in
    return _withContiguousStorage(for: new) { b in
        return CollectionDifferenceBackport(_formChanges(from: a, to: b, using:_descent(from: a, to: b)))
    }
  }
}
